...
//adds and initializes the gui toolset consisting of the matrix, sliders, clear button, and message textfield  
void setup() {
...
void myMatrix(int theX, int theY) {
  println("LED: <"+theX+", "+theY + ", " + LAYER + ", " + RED + ", " + BLUE + ", " + GREEN + ">");    //print updated cells
...
//Defines the hot keys to start/stop or clear mymatrix
void keyPressed() {
...
//control text field event and call to the corresponding character displays when enter is pressed 
void controlEvent(ControlEvent theEvent) {
  if(theEvent.isAssignableFrom(Textfield.class)) {
    String text = theEvent.getStringValue();
    println(theEvent.getName()+ "': " + text);
  ...
  for(int l = 0; l < 2; l++){              //parse intensity
    for(int h = 0; h < 8; h++){            //parse layers
      for(int i = 0; i < 8; i++){          //parse rows
        println("intensity frame (0 or 1): " + l + "  layer: " + h + "  row: " + i);
...
//Resets all the matrix values all to 0, turns all the LEDs off, and sets RGB values to 0
void reset(){
  for(int i = 0; i < 8; i++){
    for(int j = 0; j < 8; j++){
      for(int k = 0; k < 8; k++){
